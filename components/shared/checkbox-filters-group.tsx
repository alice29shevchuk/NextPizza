...
export const CheckboxFiltersGroup: React.FC<Props> = ({title,items,defaultItems,limit=5,searchInputPlaceholder='Поиск....',onChange,defaultValue,className})=> {
    ...
    <div className={className}>
            ...
            {showAll && (
                <div className='mb-5'>
                    <Input onChange={(e)=>onChangeInput(e.target.value)} placeholder={searchInputPlaceholder} className='bg-gray-50 border-none'></Input>
                </div>
            )}

            <div className='flex flex-col gap-4 max-h-96 pr-2 overflow-auto scrollbar'>
                {list.map((item,index)=>(
                    <FilterCheckbox key={index} text={item.text} value={item.value} endAdornment={item.endAdornment} checked={false} onCheckedChange={(ids)=>console.log(ids)}></FilterCheckbox>
                ))}
            </div>
            {items.length>limit && (
